...
BCD_SPARE     RMB 10; extra space for decimal and string terminator
; code section
;*******************************************************************
              ORG $4000 
Entry: 
...
NOT_START   CMPA #FWD             ; If it is not start but forward state          
            BNE NOT_FWD           ;   call forward routine                                         
            JSR FWD_ST
            JMP DISP_EXIT
            
NOT_FWD     CMPA #REV              ; If it is not forward but reverse state                                       
            BNE NOT_REV            ;  call reverse state                                       
            JSR REV_ST
            JMP DISP_EXIT

NOT_REV     CMPA #ALL_STP         ; If it is not reverse but all stop state                                         
            BNE NOT_ALL_STP       ;  call all stop state                                          
            JSR ALL_STP_ST
            JMP DISP_EXIT

NOT_ALL_STP CMPA #FWD_TRN         ;If it is all stop but forward turn state                                          
            BNE NOT_FWD_TRN       ;  call forward turn state                                          
            JSR FWD_TRN_ST
            JMP DISP_EXIT
                                              ;                                          
...
START_ST    BRCLR PORTAD0, $04, NO_FWD ;if /FWD_BUMP
            JSR INIT_FWD ;intitalize FORWARD state
            MOVB #FWD, CRNT_STATE ;Go into FORWARD state
            BRA START_EXIT   
...
NO_REAR_BUMP2  LDAA TOF_COUNTER ; If Tc>Tfwdturn then
...
; utility subroutines
;*******************************************************************
initLCD     BSET DDRB,%11111111 ; configure pins for output
...
eloop:    LDX #30 ; (2 E-clk) Initialize the inner loop counter
iloop:     
          PSHA ;2 E-clk |
          PULA ;3 E-clk |
          PSHA ;2 E-clk |
